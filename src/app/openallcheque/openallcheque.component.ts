...
@Component({
  selector: 'app-openallcheque',
  templateUrl: './openallcheque.component.html',
  styleUrls: ['./openallcheque.component.css']
})
export class OpenallchequeComponent {
  ...
  openExcell(): void {
    this.chequeService.getAllChequeexcel().subscribe((data: any[]) => {
...
onSubmit() {
  if (this.subj) {
      ...
      this.http.post<any>('http://127.0.0.1:8000/users/send_all_email/', formData, { headers: headers })
          .subscribe(
              (response) => {
